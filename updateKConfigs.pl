...
for (my $i = 0; $i < scalar(@filec); $i += 1)
{
        if ($filec[$i] =~ /name: mac81/) {
           if ($filec[($i - 1)] =~ /server: https:\/\/(.*):6443/)
           {
                $oldIp = $1;
           }
         }
...
for (my $i = 0; $i < scalar(@filec); $i += 1)
{
        ...
        if (($filec[$i] =~ /server: https:\/\/73.242.50.46:25460/)||($filec[$i] =~ /server: https:\/\/$oldIp:6443/)||($filec[$i] =~ /server: https:\/\/192.168.1.81:6443/)||($filec[$i] =~ /server: https:\/\/$newRemote:$newPort/)||($filec[$i] =~ /server: https:\/\/$newLocal:6443/))
        {
                #print $filec[($i - 1)];
